...
def quantize_gguf(input_path: str, output_path: str, quant_type: str):
    ...
    # 量化张量
    # gguf 0.19.0: 使用 GGMLQuantizationType 枚举，Q4_K 未实现 → Q4_0 替代
    from gguf import GGMLQuantizationType

    quant_enum_map = {"Q4_K_M": "Q4_0"}
    enum_name = quant_enum_map.get(quant_type, quant_type)
    qtype = getattr(GGMLQuantizationType, enum_name, None)
    if qtype is None:
        logger.error(f"Unknown quant type: {quant_type}")
        sys.exit(1)
...
def main():
    ...
    parser.add_argument("--quant", default="Q4_K_M", choices=["F16", "Q8_0", "Q4_K_M"], help="量化类型 (默认: Q4_K_M)")
